...
Source: https://handbook.gitlab.com/handbook/engineering/
...
- Engineering Workflow Code Review Security Issues Architecture Design Root Cause Analysis Strategic Priority Codes Automation GitLab Repositories Hiring Demos Cross-Functional Prioritization Developer Onboarding Engineering Communication Development Processes Development Onboarding
- Code Review
- Security Issues
- Architecture Design
- Root Cause Analysis
...
- Incident Review Page Volume - indicates the stress on our responding teams High severity incidents (S1/S2) - raises visibility of the challenges faced by all GitLab platforms
...
- Capacity Planning P1 and P2 issues and other highlights from each GitLab platform are raised.
- P1 and P2 issues and other highlights from each GitLab platform are raised.
- Action Overview Overdue S1/S2 infradev issues from each department
Include screenshots of the following graphs in the agenda . Feature Change Locks - each team in an FCL provides an update
...
- Discussions and Deep Dives - specific cross-department discussions
...
- Provides updates at the SaaS Health Review .
...
- Identify all services and feature categories the team is responsible for that are under FCL
- Identify closely coupled or dependent services and teams who may also make changes to those services or feature categories
- Notify those teams about the FCL and coordinate to ensure changes are appropriate within the scope
- Consider applying a Change Lock to your teams services to prevent unintended deployments of your service.
...
- Create a public slack channel called #fcl-incident-[number] , with members The Team’s Manager The Author and their teammates The Product Manager, the stage’s Product leader, and the section’s Product leader All reviewer(s) All maintainers(s) The chain-of-command from the manager to the VP (Sr Manager, Sr/Director, VP, etc)
- The Team’s Manager
...
- Create an FCL issue in the FCL Project with the information below in the description: Name the issue: [Group Name] FCL for Incident(s) #### Links to the incident, original change, and slack channel FCL Timeline List of work items
...
- Daily - add an update comment in your FCL issue or epic using the template: Exec-level summary Target End Date Highlights/lowlights
- Exec-level summary Target End Date Highlights/lowlights
- Target End Date
- Highlights/lowlights
- Add an agenda item in the SaaS Availability weekly standup and summarize status each week that the FCL remains open.
- Hold an asynchronous closing ceremony in the issue and/or slack channel upon completing the FCL to review the retrospectives and celebrate the learnings. Document all learnings in the issue. All FCL stakeholders and participants shall participate async. Managers of the groups participating in the FCL, including Sr. EMs and Directors should be invited. Outcome includes handbook and GitLab Docs updates where applicable.
...
- Example : A shared service configuration change occurs, and a sidekiq job from feature category X compounds the effect due to a slow query, contributing to the incident. The team owning feature category X would be subject to an FCL because their code contributed to the incident’s impact.
...
If you need to test features that are built for GitLab Ultimate then you can get added to the issue-reproduce group on production and staging environments by asking in the #development Slack channel.
...
- The issues escalated have critical business impact to the customer, determined by Customer Success and Support Engineering leadership Failure to expedite scheduling may have cascading business impact to GitLab
- Failure to expedite scheduling may have cascading business impact to GitLab
- Approval from a VP from Customer Success AND a Director of Support Engineering are required to expedite scheduling Customer Success: approval from VP, Customer Success Management - Sherrod Patching Support Engineering: approval from VP, Support - Johnny Scarborough
...
- The issue is scheduled within 1 business day For issues of type features, approval from the Product DRI is needed.
